defmodule Ancora.Review.Html do
  ...
  defp finding(finding) do
    ...
    [
      "<details class=\"finding\"><summary><span class=\"severity-marker\" aria-hidden=\"true\">",
      severity_marker(severity),
      "</span><span class=\"severity-label\">",
      Atom.to_string(severity),
      "</span> <code>",
      escape(finding.file || "unknown file"),
      "</code> <span class=\"badge ",
      Atom.to_string(severity),
      "\">",
      escape(finding.code),
  ...
  defp severity_marker(:error), do: "!"
  defp severity_marker(:warning), do: "~"
  defp severity_marker(:info), do: "i"
  ...
  defp css do
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.5 system-ui,sans-serif}code{overflow-wrap:anywhere}.shell{display:grid;grid-template-columns:260px minmax(0,1fr);min-height:100vh}aside{border-right:1px solid var(--line);padding:24px;position:sticky;top:0;height:100vh;overflow:auto}aside h1{font-size:18px}aside h2{font-size:11px;text-transform:uppercase;color:var(--muted);margin-top:24px}nav a{display:block;color:var(--ink);padding:6px 0;text-decoration:none}main{width:min(1120px,100%);padding:42px;margin:auto}section{scroll-margin-top:24px;margin-bottom:56px}.subject,.panel,.file{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:20px;margin:14px 0}header{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.eyebrow{color:var(--muted);font-size:11px;letter-spacing:.12em;text-transform:uppercase;margin:0}.chip,.badge{border:1px solid currentColor;border-radius:999px;padding:3px 9px;font-size:12px}.pass,.acknowledged{color:var(--ok)}.fail,.error,.drift{color:var(--bad)}.warning{color:var(--warn)}.info{color:var(--accent)}.meta,.empty{color:var(--muted)}.delta-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.pivots{display:flex;gap:6px;border-bottom:1px solid var(--line);margin-top:20px}.pivots button{appearance:none;border:0;background:none;color:var(--muted);padding:10px 14px}.pivots button.active{color:var(--accent);border-bottom:2px solid var(--accent)}.pivot{display:none;padding-top:16px}.pivot.active{display:block}.claim,.watched{border-left:3px solid var(--line);padding:8px 14px;margin:10px 0}.diff{overflow:auto;background:#111;color:#ddd;padding:14px;border-radius:7px;font-size:12px}.diff > code > span{display:block}.diff .add{color:#8ee3ab;background:#163522}.diff .del{color:#ff9b92;background:#3c1e1b}.diff .hunk_header{color:#a99cff}.finding{display:inline-block;margin:4px}.finding p{max-width:720px}.severity-marker{display:inline-block;font-weight:700;min-width:1em;text-align:center}.severity-label{font-weight:700}.summary{max-width:76ch}@media(max-width:800px){.shell{display:block}aside{position:static;height:auto;border-right:0;border-bottom:1px solid var(--line)}main{padding:22px}.delta-grid{grid-template-columns:1fr}}
    .drift_transitive{color:var(--accent)}
    """
  end
end
